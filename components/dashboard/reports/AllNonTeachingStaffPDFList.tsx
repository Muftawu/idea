...
const initials = (student: NonTeachingStaffSchemaT) =>
...
const PhotoCell = ({ item }: { item: NonTeachingStaffSchemaT }) => (
    <View style={s.photoBox}>
        <Text style={s.photoPlaceholderText}>{initials(item)}</Text>
    </View>
);

const DataRow = ({ item, index }: { item: NonTeachingStaffSchemaT; index: number }) => (
    <View style={[s.tableRow, index % 2 === 1 ? s.tableRowAlt : {}]} wrap={false}>
        ...
        <View style={s.colIndex}>
            <Text style={s.tdIndex}>IS{item.staffId || "IS---"}</Text>
        </View>

        <View style={s.colName}>
            <Text style={s.tdNameSurname}>{item.surname} {item.otherNames}</Text>
            {/* <Text style={s.tdNameOther}>{student.otherNames}</Text> */}
        </View>

        <View style={s.colGender}>
            <GenderPill gender={item.gender ?? "m"} />
        </View>

        <View style={s.colDob}>
            <Text style={s.tdDob}>{item.jobDescription || "N/A"}</Text>
        </View>
...
export const AllNonTeachingStaffPDFList = ({ academicTerm, academicYear, data }: { academicTerm: string, academicYear: string, data: NonTeachingStaffSchemaT[] }) => {

    const males = data.filter((item) => isMale(item.gender)).length;
    const females = data.length - males;

    return (
        <Document
            title={`All Non-Teaching Staff List — ${new Date().toDateString()}`}
            author="IDEA International School"
            subject="All Non Teaching Staff List"
        >
            <Page size="A4" style={s.page} orientation="portrait">
                ...
                <View style={s.headerBand} fixed>
                    ...
                    <View style={s.reportBadge}>
                        <Text style={s.reportBadgeTop}>STAFF REGISTER</Text>
                        <Text style={s.reportBadgeMain}>All Non-Teaching Staff</Text>
                    </View>
                </View>
                ...
                <View style={s.body}>
                    ...
                    <View style={s.infoCard}>
                        <View style={s.infoCardHeader}>
                            ...
                            <Text style={s.infoCardTitle}>NON-TEACHING STAFF INFORMATION</Text>
                        </View>
                    ...
                    < View style={s.table} >
                        ...
                        < View style={s.tableHeaderRow} fixed >
                            ...
                            <View style={s.colDob}>
                                <Text style={s.thText}>JOB ROLE</Text>
                            </View>
                            {/* <View style={s.colSection}> */}
