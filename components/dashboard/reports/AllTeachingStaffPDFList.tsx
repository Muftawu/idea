...
const initials = (student: StaffT) =>
    `${student.personalInfo.last_name?.[0] ?? ""}${student.personalInfo.first_name?.[0] ?? ""}`.toUpperCase();
...
const PhotoCell = ({ item }: { item: StaffT }) => (
    <View style={s.photoBox}>
        <Text style={s.photoPlaceholderText}>{initials(item)}</Text>
    </View>
);

const DataRow = ({ item, index }: { item: StaffT; index: number }) => (
    <View style={[s.tableRow, index % 2 === 1 ? s.tableRowAlt : {}]} wrap={false}>
        ...
        <View style={s.colName}>
            <Text style={s.tdNameSurname}>{item.personalInfo.last_name} {item.personalInfo.first_name}</Text>
            {/* <Text style={s.tdNameOther}>{student.otherNames}</Text> */}
        </View>

        <View style={s.colGender}>
            <GenderPill gender={item.personalInfo.gender ?? "m"} />
        </View>

        <View style={s.colDob}>
            <Text style={s.tdDob}>{item.assignedClasses?.map((obj) => obj.name) || "N/A"}</Text>
        </View>
...
export const AllTeachingStaffPDFList = ({ academicTerm, academicYear, data }: { academicTerm: string, academicYear: string, data: StaffT[] }) => {

    const males = data.filter((item) => isMale(item.personalInfo.gender)).length;
            ...
            <Page size="A4" style={s.page} orientation="portrait">
                ...
                <View style={s.headerBand} fixed>
                    ...
                    <View style={s.reportBadge}>
                        ...
                        <Text style={s.reportBadgeMain}>All Teaching Staff</Text>
                    </View>
                </View>
                ...
                <View style={s.body}>
                    ...
                    <View style={s.infoCard}>
                        <View style={s.infoCardHeader}>
                            ...
                            <Text style={s.infoCardTitle}>TEACHING STAFF INFORMATION</Text>
                        </View>
