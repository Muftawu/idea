...
/**
 * IDEA International School — Modern Achievement Report Card
 * Built with @react-pdf/renderer
 *
 * Usage:
 *   import { ReportCard, DownloadReportButton } from "./IdeaSchoolReport"
 *
 *   <DownloadReportButton data={reportData} />
 *   // or render inline:
 *   <PDFViewer width="100%" height="800px"><ReportCard data={reportData} /></PDFViewer>
 *
 * Install deps:
...
export const ReportCard = ({ data }: { data: ReportCardData }) => (
  <Document
    title={`${data.studentName} — ${data.term} Report`}
      ...
      <View style={s.body}>
        ...
        <View style={s.infoCard}>
          ...
          <View style={s.infoGrid}>
            <InfoCell label="Full Name" value={val(data.studentName)} wide />
            <InfoCell label="Class" value={val(data.className)} />
            <InfoCell label="Term" value={val(data.term)} />
            <InfoCell label="Academic Year" value={val(data.year)} />
            <InfoCell label="No. on Roll" value={val(data.rollNumber)} />
            <InfoCell label="Vacation Date" value={val(data.vacationDate)} />
            <InfoCell label="Reopening Date" value={val(data.reopeningDate)} />
          </View>
        </View>
...
// ─── Sample Data (mirrors the original report) ────────────────────────────────
...
// export const DownloadReportButton = ({ data }: { data: ReportCardData }) => (
//   <PDFDownloadLink
//     document={<ReportCard data={data} />}
//     fileName={`${data.studentName.replace(/\s+/g, "_")}_${data.term.replace(/\s+/g, "_")}_Report.pdf`}
//   >
//     {({ loading }) => (loading ? "Generating PDF…" : "Download Report Card")}
//   </PDFDownloadLink>
// );
